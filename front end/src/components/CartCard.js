		...
		<div className="flex gap-2 md:gap-4 border border-slate-300 ">
			...
			<div className="flex flex-col py-1  md:py-2 w-full pr-3 ">
				<div className="flex justify-between w-full">
					...
					<div
						className="text-slate-700 hover:text-white hover:bg-red-600 md:p-1 rounded-full cursor-pointer"
						onClick={() => dispatch(deleteItemCart({ _id: id }))}>
						<MdOutlineDelete />
					</div>
				</div>
				...
				<div className="md:my-1 flex">
					<div className="flex ">
						<button
							className="bg-slate-200 w-6 h-6 md:w-7 md:h-7 flex items-center justify-center text-lg rounded"
							onClick={() => dispatch(increaseQty({ _id: id }))}>
							<HiPlusSm />
						</button>
						...
						<button
							className="bg-slate-200 w-6 h-6 md:w-7 md:h-7 flex items-center justify-center text-lg rounded"
							onClick={() => dispatch(decreaseQty({ _id: id }))}>
							<HiMinusSm />
						</button>
					</div>
					<div className="flex ml-auto text-slate-700">
						...
						<p className="font-medium">{totalPriceUK}</p>
					</div>
				</div>
			</div>
		</div>
